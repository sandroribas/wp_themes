<?php
//load_theme_textdomain('2br', get_template_directory() . '/languages');
...
$args = array(
	'width'         => 335,
	'height'        => 101,
	'default-image' => get_template_directory_uri() . '/img/logo-topo.png',
	'uploads'       => true,
);
...
function minha_tela_de_login() {
  ...
  ' . WP_CONTENT_URL . '/themes/2br/css/meu_login.css" />';
}
...
function change_footer_version() {return 'TWOBrasil CMS Admin - Versão 1.0';}
...
/*---Registrando Formulário de busca personalizado. Forma de Uso: <?php get_search_form('minha_busca'); ?>---*/
 function busca_topo( $form ) {
	$form = '<form method="get" id="searchform" action="'. home_url( '/' ).'">
  				<div id="busca">
    				<input type="text" class="field" name="s" id="s" placeholder="'. esc_attr_e( '', '2br' ).'" />
					<input type="submit" class="submit" name="submit" id="searchsubmit" value="'. esc_attr_e( '', '2br' ).'" />
  				</div>
			</form>';
...
//---GOOGLE FONTS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!---------------------------------------------------------------------------------
function load_fonts() {
            wp_register_style('googleFonts', 'http://fonts.googleapis.com/css?family=Gentium+Basic|Open+Sans|Open+Sans+Condensed:300|Gentium+Book+Basic');
